...
export class MoveActionBundle
	implements
		IBundledInputCommand<FileTreeActionID, MoveActionInput, MoveActionResult, FileTreeActionError>
{
	public readonly descriptor: FileTreeActionDescriptor;
	public readonly availability: Readable<
		InputCommandAvailability<MoveActionInput, FileTreeActionError>
	>;
	public readonly commandContext: Readable<FileTreeCommandContext>;

	private readonly primitive: IFileTreeInputAction<MoveActionInput, MoveActionResult>;
	...
	constructor(
		primitive: IFileTreeInputAction<MoveActionInput, MoveActionResult>,
	...
	public canPerform(performInput: MoveActionInput): Result<void, FileTreeActionError> {
	...
	public async perform(
		...
		const commandContext: FileTreeCommandContext = buildSelectionContext(selectedNodeID);
		const result: Result<MoveActionResult, FileTreeActionError> = await this.primitive.perform(
			commandContext,
			performInput
		);
		return result;
	...
	> {
		...
		> = derived(
			...
			([selectedNodeID, _fsMap]: [
				...
				if (selectedNodeID === null) {
					const error: FileTreeActionError = this.errorFactory.createMissingSelectionError();
					const unavailable: InputCommandAvailability<MoveActionInput, FileTreeActionError> = {
				...
				const commandContext: FileTreeCommandContext = buildSelectionContext(selectedNodeID);
				const result: InputCommandAvailability<MoveActionInput, FileTreeActionError> =
