...
export class CreateFolderHandler implements ICommandHandler<CreateFolderCommand> {
	...
	public execute(
		...
		const folderName: string = command.name;
		...
		const nameValidation: ValidationResult<void> = validateNodeName(folderName);
		...
		const nodeSpec: FolderNodeSpec = this.nodeFactory.createFolderNode(
			parentID,
			folderName,
			DEFAULT_PERMISSIONS,
			effectiveUserSpace
