...
<div
	...
	class={cn(
		'group relative flex min-h-7 w-full items-center gap-2 text-left text-[13px] transition-colors duration-100',
		'hover:bg-sidebar-accent',
		(item.isSelected || isActive) && 'bg-sidebar-accent',
...
>
	{#if isActive}
		<div class="absolute bottom-0.5 left-0 top-0.5 w-0.5 rounded-full bg-primary"></div>
	{/if}

	{#if item.type === NodeType.FOLDER}
		<button
			class="flex size-4 shrink-0 items-center justify-center rounded-sm text-muted-foreground transition-colors hover:text-foreground"
			onclick={handleTwistyClick}
		>
			<ChevronRight
				class={cn('size-3.5 transition-transform duration-150', isExpanded && 'rotate-90')}
			/>
		</button>
	{:else}
	...
	{#if item.type === NodeType.FILE}
		{#if saveStatus === FileSaveStatus.SAVEABLE}
			<span class="ml-auto mr-2 flex shrink-0 items-center" title={messages.commonStatusUnsaved}>
				<CircleDot class="size-3.5 text-sidebar-foreground" />
			</span>
